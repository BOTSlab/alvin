""" RegionSampleCam was forked from PointSampleCam to provide a much more straightforward sensor that detects whether an object of the desired type is close to a sample position (xr, yr).  A data file is read which gives the positions of these sample points. """
...
class RegionSampleImage:
    def __init__(self, calib_array):
...
class RegionSampleCam:
    def __init__(self, calib_filename, detection_mask, acceptance_mask):
        """
        config = ConfigSingleton.get_instance()
        self.min_distance = config.getfloat("RegionSampleCam", "min_distance")
        self.max_distance = config.getfloat("RegionSampleCam", "max_distance")
        self.max_abs_angle = config.getfloat("RegionSampleCam", "max_abs_angle")
        """
    ...
    def compute(self, env, robot):
        ...
        image = RegionSampleImage(self.calib_array, self.neighbour_array)
...
if __name__ == '__main__':
    print "RUN"
    sampler = RegionSampler("../data/phase1_160x120.csv", 0, 0)
